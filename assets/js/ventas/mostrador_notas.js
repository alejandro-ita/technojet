jQuery(function($) {
	...
	initDataTable('#tbl-pi-mostrador', {
		...
		ajax: {
			url: base_url('ventas/ventas/get_nc_mostrador'),
	...
	initDataTable('#tbl-nc-mostrador-consecutivo', {
	...
	.on('click' , '.tools-tbl-pi .add-cotizacion', function(e) {
		...
		$('#form-filtro').formAjaxSend({
			url: base_url('ventas/ventas/get_modal_add_mostrador_notas'),
			dataType: 'html',
		...
		if ($('#modal-nuevo-pi-mostrador form').valid()) {
			...
			if(rows.length) {
				...
				$('#modal-nuevo-pi-mostrador form').formAjaxSend({
					url: base_url('ventas/ventas/process_save_nc_mostrador'),
					data: {
					...
					success: function(response) {
						if(response.success) {
							...
							IS.init.dataTable['tbl-nc-mostrador-consecutivo'].ajax.reload(function(jsonData) {
							    IS.init.dataTable['tbl-nc-mostrador-consecutivo'].columns.adjust().draw();
							});
	...
	.on('click', '#tbl-pi-mostrador button#open-modal-update', function(e) {
		...
		$.formAjaxSend({
			url: base_url('ventas/ventas/get_modal_edit_nc'),
			data: tr.data(),
			dataType: 'html',
			success: function(modal) {
				$('#content-modals').html(modal);
		...
		if ($('#modal-editar-pi-mostrador form').valid()) {
			...
			if(rows.length) {
				...
				$('#modal-editar-pi-mostrador form').formAjaxSend({
					url: base_url('ventas/ventas/process_update_nc'),
					data: {
						/*tipo_requisicion: $("#id_tipo_requisicion option:selected").text(),
						vale_entrada: $("#id_vale_entrada option:selected").text(),
						departamento_solicitante: $("#id_departamento_solicitante option:selected").text(),
	...
	.on('click', '#tbl-pi-mostrador button#remove', function(e) {
		...
		ISswal.fire({
		    ...
		    if (result.value) {
		    	...
		    	$.formAjaxSend({
		    		url: base_url('ventas/ventas/process_remove_nc'),
		    		data: data,
		    		success: function(response) {
		    			if(response.success) {
							ISToast.fire({icon: response.icon, title: response.msg, customClass: response.icon});
...
/*jQuery(function($) {
	$('body')
	.on('click' , '.add-mostrador', function(e) {
		$('#form-filtro').formAjaxSend({
			url: base_url('ventas/ventas/get_modal_add_mostrador_notas'),
			dataType: 'html',
			success: function(modal) {
				$('#content-modals').html(modal);
				initModal('#modal-nuevo-entrada-mostrador-notas', {
					onOpenEnd: function() {
						initSelect2('.modal select');
						$('#modal-add-producto-entrada-notas').validate();
						init_tbl_entrada_nuevos_productos();
					},
					onCloseEnd: function() {
						$('#modal-nuevo-entrada-mostrador-notas').remove();
	...
	.on('click', '#close-mostrador',function(e){
		$('#modal-nuevo-entrada-mostrador-notas').remove();
		$('.modal-backdrop').remove();
	})
	...
	.on('click', '#modal-tbl-entrada-productos_filter .btn-add', function(e) {
		...
		$('#form-filtro').formAjaxSend({
			url: base_url('ventas/ventas/get_modal_add_mostrador_notas_product'),
			dataType: 'html',
			success: function(modal) {
				$('#content-modals').append(modal);
	...
	.on('click', '#close-mostrador-product',function(e){
		$('#modal-add-producto-entrada').remove();
	})
