jQuery(function($) {
	initDataTable('#tbl-requisicion-material', {
		ajax: {
		 	 url: base_url('ventas/requisicion_material/get_productos_requisicion_material')
		}
	...
	.on('click', '#tbl-requisicion-material button#open-modal-update', function(e) {
		...
		$.formAjaxSend({
			 url: base_url('ventas/requisicion_material/get_modal_edit_requisicion')
			,data: tr.data()
			,dataType: 'html'
	...
	.on('click', '#tbl-requisicion-material button#remove', function(e) {
		...
		ISswal.fire({
		    ...
		    if (result.value) {
		    	...
		    	$.formAjaxSend({
		    		url: base_url('ventas/requisicion_material/process_remove_requisicion'),
		    		data: data,
	...
	.on('click', '#tbl-requisicion-material button#build-pdf', function(e) {
    	...
    	$.formAjaxSend({
    		url: base_url('ventas/requisicion_material/process_build_pdf_requsicion'),
    		data: tr.data(),
    		success: function(response) {
    			if(response.success) {
	...
	.on('click', '.tools-tbl-requisicion .add-requisicion', function(e) {
		...
		$('#form-filtro').formAjaxSend({
			 url: base_url('ventas/requisicion_material/get_modal_add_requisicion')
			,dataType: 'html'
			,success: function(modal) {
				$('#content-modals').html(modal);
	...
	.on('click', '#modal-tbl-productos-requisicion_filter .btn-add', function(e) {
		$.formAjaxSend({
			 url: base_url('ventas/requisicion_material/get_modal_add_producto_requisicion')
			,dataType: 'html'
			,success: function(modal) {
		...
		if ($('#modal-nueva-vale-requisicion form').valid()) {
			...
			if(rows.length) {
				...
				$('#modal-nueva-vale-requisicion form').formAjaxSend({
					 url: base_url('ventas/requisicion_material/process_save_productos_requisicion')
					,data: {
		...
		if ($('#modal-editar-vale-requisicion form').valid()) {
			...
			if(rows.length) {
				...
				$('#modal-editar-vale-requisicion form').formAjaxSend({
					 url: base_url('ventas/requisicion_material/process_update_productos_requisicion')
					,data: {
						 tipo_requisicion: $("#id_tipo_requisicion option:selected").text()
						,vale_entrada: $("#id_vale_entrada option:selected").text()
						,departamento_solicitante: $("#id_departamento_solicitante option:selected").text()
