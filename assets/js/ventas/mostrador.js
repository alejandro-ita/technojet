jQuery(function($) {
	...
	.on('keyup', '.tools-tbl-pi-consecutivo #buscar', function(e) {
		IS.init.dataTable['tbl-pi-mostrador-consecutivo'].search(this.value).draw();
	})

	.on('click', '.tools-tbl-pi-consecutivo .pi_consecutivo_length a', function(e) {
		$(this).closest('ul').find('li').removeClass('active');
		$(this).parent('li').addClass('active');
	...
	.on('click', '#tbl-pi-mostrador button#build-pdf', function(e) {
    	...
    	$.formAjaxSend({
    		url: base_url('ventas/ventas/process_build_pdf_pi_mostrador'),
    		data: tr.data(),
    		success: function(response) {
    			if(response.success) {
					gotoLink(base_url(response.file_path));
