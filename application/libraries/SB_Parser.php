 ...
 * @since   Version 1.0.0
 * @link https://github.com/gccloud/parser Parser Conditional
 */
 ...
 * @version     20161121
 */
class SB_Parser extends CI_Parser
{
    ...
    protected function _parse($template, $data, $return = FALSE)
    {
        ...
        foreach ($replace as $from => $to) {
            ...
            $template = str_ireplace($from, ( ! is_null($to) ? $to : $from), $template); #PARCHE BY IS
        }
            ...
            foreach ($helpers as $helper) {
                ...
                if ($func === 'empty') {
                    ...
                    try {
                        #PARCHE BY IS
                        foreach ($args as &$tpl) {
                            $tpl = self::_remove_unparsed($tpl);
                            $key = str_replace([$this->l_delim, $this->r_delim], '', $tpl);
                            $tpl = $this->parse_string($tpl, [$key=>''], TRUE);
                        }
                        #END PARCHE BY IS
        ...
        foreach ($matches as $match) {
            ...
            foreach ($data as $pos => $row) {
                ...
                foreach ($row as $key => $val) {
                    ...
                    $temp[$this->l_delim.$key.$this->r_delim] = ( ! is_null($val) ? $val : $this->l_delim.$key.$this->r_delim); #PARCHE BY IS
                }
    ...
    protected function _replace_unparsed($template)
    {
        ...
        if ( ! empty($unparsed)) {
            foreach ($unparsed as $u) {
                ...
                $template = str_ireplace($u[0], str_ireplace([$this->l_delim, $this->r_delim], [$this->ISl_delim, $this->ISr_delim], $u[0]), $template); #PARCHE BY IS
            }
        }
    ...
    protected function _remove_unparsed($template)
    {
        ...
        return str_ireplace([$this->ISl_delim, $this->ISr_delim], [$this->l_delim, $this->r_delim], $template); #PARCHE BY IS
    }

}


/* End of file IS_Parser.php */
/* Location: ./application/libraries/IS_Parser.php */
