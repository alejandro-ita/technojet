...
class Vendedores_model extends SB_Model {

	public function get_vendedores_main(array $where = [], $all=TRUE) {
		$tbl = $this->tbl;

        !isset($where['notIN']) OR $this->db->where_not_in('VEN.id_vendedor', $where['notIN']);
		!isset($where['id_vendedor']) OR $this->db->where('VEN.id_vendedor', $where['id_vendedor']);
		!isset($where['vendedor']) OR $this->db->where('VEN.vendedor', $where['vendedor']);

		$request = $this->db->select("VEN.*", FALSE)
			->from("$tbl[vendedores] AS VEN")
			->where('VEN.activo', 1)
			->get();
	...
	public function insert_vendedor(array $data, $batch=FALSE) {
		$tbl = $this->tbl;

		if (!$batch) {
			$data['id_usuario_insert'] 	= $this->session->userdata('id_usuario');
			$data['timestamp_insert'] 	= timestamp();
			$this->db->insert($tbl['vendedores'], $data);
		} else $this->db->insert_batch($tbl['vendedores'], $data);
	...
	public function update_vendedor(array $data, array $where, $affectedRows=TRUE) {
		$tbl = $this->tbl;

		$data['id_usuario_update'] 	= $this->session->userdata('id_usuario');
		$data['timestamp_update'] 	= timestamp();
		$this->db->update($tbl['vendedores'], $data, $where);
