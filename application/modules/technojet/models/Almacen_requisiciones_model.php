...
class Almacen_requisiciones_model extends SB_Model {

	public function get_almacen_requisicion_min(array $where, $all=TRUE) {
		$tbl = $this->tbl;

		!isset($where['notIN']) OR $this->db->where_not_in('TAR.id_almacen_requisicion', $where['notIN']);
		!isset($where['id_almacen_requisicion']) OR $this->db->where('TAR.id_almacen_requisicion', $where['id_almacen_requisicion']);
		!isset($where['id_categoria']) OR $this->db->where('TAR.id_categoria', $where['id_categoria']);
		!isset($where['requisicion']) OR $this->db->where('TAR.requisicion', $where['requisicion']);
		!isset($where['grupo']) OR $this->db->where('CC.grupo', $where['grupo']);
		$request = $this->db->select("
				 TAR.*,
				,CC.categoria
			", FALSE)
			->from("$tbl[almacen_requisiciones] AS TAR")
	...
	public function insert_almacen_requisicion(array $data, $batch=FALSE) {
		$tbl = $this->tbl;

		if (!$batch) {
			$data['id_usuario_insert'] 	= $this->session->userdata('id_usuario');
			$data['timestamp_insert'] 	= timestamp();
			$this->db->insert($tbl['almacen_requisiciones'], $data);
		} else $this->db->insert_batch($tbl['almacen_requisiciones'], $data);
	...
	public function update_almacen_requisicion(array $data, array $where, $affectedRows=TRUE) {
		$tbl = $this->tbl;

		$data['id_usuario_update'] 	= $this->session->userdata('id_usuario');
		$data['timestamp_update'] 	= timestamp();
		$this->db->update($tbl['almacen_requisiciones'], $data, $where);
