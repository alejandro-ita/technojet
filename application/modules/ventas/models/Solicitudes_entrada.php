...
class Solicitudes_entrada extends SB_Model {
	...
	public function get_solicitudes_main(array $where = [], $all=TRUE) {
		...
		$request = $this->db->select("
			...
			(SELECT c_cotizacion FROM $tbl[ventas_cotizaciones] as VC WHERE VC.id_ventas_cotizacion = CT.id_almacen_saliente) as almacen_saliente,
			CT.id_almacen_saliente,
	...
	public function insert_solicitud(array $data, $batch=FALSE) {
		...
		if (!$batch) {
			...
			$this->db->insert($tbl['solicitud_entrega'], $data);
		} else $this->db->insert_batch($tbl['solicitud_entrega'], $data);
	...
	public function update_solicitud(array $data, array $where, $affectedRows=TRUE) {
		...
		$this->db->update($tbl['solicitud_entrega'], $data, $where);
	...
	public function insert_producto(array $data, $batch=TRUE) {
		...
		if (!$batch) {
			$data['id_usuario_insert'] 	= $this->session->userdata('id_usuario');
			$data['timestamp_insert'] 	= timestamp();
			$this->db->insert($tbl['solicitud_entrega_prod'], $data);
		} else $this->db->insert_batch($tbl['solicitud_entrega_prod'], $data);
	...
	public function update_solicitud_productos(array $data, array $where, $affectedRows=TRUE) {
		...
		if (isset($where['notIn'])) {
			$this->db->where_not_in('id_solicitud_entrega_productos', $where['notIn']);
			unset($where['notIn']);
		}

		$data['id_usuario_update'] = $this->session->userdata('id_usuario');
		$data['timestamp_update'] = timestamp();
		$this->db->update($tbl['solicitud_entrega_prod'], $data, $where);
	...
	public function get_solicitud_productos(array $where=[], $all=TRUE) {
		...
		$request = $this->db->select("
				TRP.id_solicitud_entrega_producto,
				TRP.id_solicitud,
				TRP.id_producto,
				TRP.cantidad,
				CTP.tipo_producto,
			...
			->from("$tbl[solicitud_entrega_prod] AS TRP")
